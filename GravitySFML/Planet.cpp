...
void Planet::set_accel(sf::Vector2f acc)
{
	accel = acc;
}
...
sf::Vector2f Planet::get_position()
{
	return position;
}
...
int Planet::get_size()
{
	return actual_planet.getRadius();
}

sf::Color Planet::get_color()
{
	return  actual_planet.getFillColor();
}
...
void Planet::draw_planet(sf::RenderWindow* window)
{
	window->draw(actual_planet);
}